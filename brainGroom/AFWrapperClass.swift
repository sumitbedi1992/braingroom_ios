...
// MARK: Alamofire requestPOSTURLWithUrlsession:
    ...
    class func requestWithArr(_ strURL : String, params : NSArray, success:@escaping (NSDictionary) -> Void, failure:@escaping (NSError) -> Void)
    {
                let urlwithPercentEscapes = strURL.addingPercentEncoding( withAllowedCharacters: CharacterSet.urlQueryAllowed)
...
// MARK: Alamofire Multiform data post api :
    ...
    {
        let alert = UIAlertController(title:"", message: "SHARE", preferredStyle: UIAlertControllerStyle.alert)
